...
class ReportGenerationView(ttk.Frame):
    ...
    def show_final_reports(self):
        ...
        if self.app_container.is_custom_division == False:
            self.division_detail_label = ttk.Label(self.final_output_labelframe, text='  division detail report.pdf', style="Hyperlink.TLabel", cursor="hand2")
            self.division_detail_label.grid(row=1, column=0, sticky=tk.W, **options)
            division_detail_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'DivisionDetailReport.pdf')
            division_detail_report_filename_with_path = 'file://' + str(division_detail_disk_path)
        ...
        self.kata_score_sheet_lable = ttk.Label(self.final_output_labelframe, text='  kata score sheet.pdf', style="Hyperlink.TLabel", cursor="hand2")
        self.kata_score_sheet_lable.grid(row=2, column=0, sticky=tk.W, **options)
        kata_score_sheet_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'KataScoreSheet.pdf')
        kata_score_sheet_filename_with_path = 'file://' + str(kata_score_sheet_disk_path)
        ...
        if self.app_container.is_custom_division == False:
            self.technique_score_sheet_lable = ttk.Label(self.final_output_labelframe, text='  technique score sheet.pdf', style="Hyperlink.TLabel", cursor="hand2")
            self.technique_score_sheet_lable.grid(row=3, column=0, sticky=tk.W, **options)
            technique_score_sheet_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'TechniqueScoreSheet.pdf')
            technique_score_sheet_filename_with_path = 'file://' + str(technique_score_sheet_disk_path)
        ...
        self.sparring_tree_letter_size_label = ttk.Label(self.final_output_labelframe, text='  sparring tree - letter size.pdf', style="Hyperlink.TLabel", cursor="hand2")
        self.sparring_tree_letter_size_label.grid(row=4, column=0, sticky=tk.W, **options)
        sparring_tree_letter_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'SparringTreeReport-Letter.pdf')
        sparring_tree_letter_filename_with_path = 'file://' + str(sparring_tree_letter_disk_path)
        ...
        if self.app_container.is_custom_division == False:
            sparring_tree_legal_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'SparringTreeReport-Legal.pdf')
            sparring_tree_legal_filename_with_path = 'file://' + str(sparring_tree_legal_disk_path)
            if os.path.exists(sparring_tree_legal_disk_path):
                self.sparring_tree_legal_size_label = ttk.Label(self.final_output_labelframe, text='  sparring tree - legal size.pdf', style="Hyperlink.TLabel", cursor="hand2")
        ...
        if self.app_container.is_custom_division == False:
            self.envelope_report_label = ttk.Label(self.final_output_labelframe, text='  envelope report.pdf', style="Hyperlink.TLabel", cursor="hand2")
            self.envelope_report_label.grid(row=7, column=0, sticky=tk.W, **options)
            envelope_report_disk_path = pathlib.Path(self.app_container.tournament_output_folder_path / 'EnvelopeReport.pdf')
            envelope_report_report_filename_with_path = 'file://' + str(envelope_report_disk_path)
            self.envelope_report_label.bind('<Button-1>', lambda x: webbrowser.open(envelope_report_report_filename_with_path))
